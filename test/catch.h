        ...
        class BasicResult : public ResultValueBase<T>
            ...
            inline auto convertInto (std::string const &source, bool &target) -> ParserResult
            {
                ...
                { target = true; }
                    ...
                    if (srcLC == "n" || srcLC == "0" || srcLC == "false" || srcLC == "no" || srcLC == "off")
                    { target = false; }
    ...
    clara::Parser makeCommandLineParser (ConfigData& config)
    {
        ...
        auto const setTestOrder = [&] (std::string const & order)
        {
            ...
            { config.runOrder = RunTests::InDeclarationOrder; }
                ...
                if (startsWith ("lexical", order))
                { config.runOrder = RunTests::InLexicographicalOrder; }
                
                else
                    if (startsWith ("random", order))
...
namespace Catch
{
    ...
    void TestSpecParser::visitChar (char c)
    {
        ...
        if (m_mode == Name)
        {
            ...
            if (m_mode == EscapedName)
            { m_mode = Name; }
            
            else
                if (m_mode == QuotedName && c == '"')
                { addPattern<TestSpec::NamePattern>(); }
...
namespace Catch
{
    ...
    Totals Totals::delta (Totals const& prevTotals) const
    {
        ...
        { ++diff.testCases.failed; }
            ...
            if (diff.assertions.failedButOk > 0)
        ...
        private:
            ...
            void printTotals (const Totals& totals) const
            {
                if (totals.testCases.total() == 0)
                {
                    ...
                    if (totals.testCases.failed == totals.testCases.total())
                    {
                      ...
                      m_units (units)
                {
                    if (m_units == Unit::Auto)
                    {
                        ...
                        { m_units = Unit::Nanoseconds; }
                            ...
                            if (m_inNanoseconds < s_nanosecondsInAMillisecond)
                            { m_units = Unit::Microseconds; }
                            
                            else
                                if (m_inNanoseconds < s_nanosecondsInASecond)
                                { m_units = Unit::Milliseconds; }
                                
                                else
                                    if (m_inNanoseconds < s_nanosecondsInAMinute)
